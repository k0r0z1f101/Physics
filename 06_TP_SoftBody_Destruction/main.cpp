...
int main(int, char**)
{
	...
	if (pWinSettings)
	{
		oCanvas.SetView(oCanvas.ViewOrigin() + ImVec2(static_cast<f32>(pWinSettings->Size.x) * 0.5f, static_cast<f32>(pWinSettings->Size.y) * 0.95f), oCanvas.ViewScale());
	}
	else
	{
		oCanvas.SetView(oCanvas.ViewOrigin() + ImVec2(0.0f, 300.0f), oCanvas.ViewScale());
	}
