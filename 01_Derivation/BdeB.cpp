...
f32 Function(f32 const x)
{
	...
	//return x * x;
	//return Funcf::Sin(x / Funcf::Cos(x));
}
...
f32 DFunction(f32 const x)
{
	...
	//return 2.0f * x;
	//return Funcf::Cos(x / Funcf::Cos(x)) / Funcf::Cos(x) * (1.0f + x * Funcf::Tan(x));
}

f32 DDFunction(f32 const x, f32 const dt)
{
#if 0
	switch (eCurNumericalDiff)
	{
	case Centered:
		return (0.5f * DFunction(x + dt) - 0.5f * DFunction(x - dt)) / dt;
	case Forward:
		return (DFunction(x + dt) - DFunction(x)) / dt;
	case Backward:
		return (DFunction(x) - DFunction(x - dt)) / dt;
	case CenteredOrder6:
	default:
		// Sixth order accuracy - 1st derivative
		return ((-1.0f / 60.0f) * DFunction(x - 3.0f * dt)
			+ (3.0f / 20.0f) * DFunction(x - 2.0f * dt)
			- (3.0f / 4.0f) * DFunction(x - dt)
			+ (3.0f / 4.0f) * DFunction(x + dt)
			- (3.0f / 20.0f) * DFunction(x + 2.0f * dt)
			+ (1.0f / 60.0f) * DFunction(x + 3.0f * dt)) / dt;
	}
#else
	switch (eCurNumericalDiff)
	{
	case Centered:
		return (Function(x - dt) - 2.0f * Function(x) + Function(x + dt)) / (dt * dt);
	case Forward:
		return (Function(x) - 2.0f * Function(x + dt) + Function(x + 2.0f * dt)) / (dt * dt);
	case Backward:
		return (Function(x - 2.0f * dt) - 2.0f * Function(x - dt) + Function(x)) / (dt * dt);
	case CenteredOrder6:
	default:
		// Sixth order accuracy - 2st derivative
		return ((1.0f / 90.0f) * Function(x - 3.0f * dt)
			- (3.0f / 20.0f) * Function(x - 2.0f * dt)
			+ (3.0f / 2.0f) * Function(x - dt)
			- (49.0f / 18.0f) * Function(x)
			+ (3.0f / 2.0f) * Function(x + dt)
			- (3.0f / 20.0f) * Function(x + 2.0f * dt)
			+ (1.0f / 90.0f) * Function(x + 3.0f * dt)) / (dt * dt);
	}
#endif
}
...
void DrawScene(ImDrawList* pDrawList, f32 const dt)
{
	...
	if (bShowLocalDiff)
	{
		...
		// Forward
		f32 const fDYF = (Function(fPosition + fLocalDelta) - Function(fPosition)) / fLocalDelta;
		// Backward
		f32 const fDYB = (Function(fPosition) - Function(fPosition - fLocalDelta)) / fLocalDelta;
		// Sixth order accuracy - 1st derivative
	...
	if (bShowIntegration)
	{
		f32 fComputedY0 = Function(fPosition - fDeltaTime);
		...
		{
			f32 const fDDY = DDFunction(fTheta, fDeltaTime);
			f32 const fNewY = 2.0f * fComputedY - fComputedY0 + fDeltaTime * fDeltaTime * fDDY;

			pDrawList->AddLine(VecToImDraw(fTheta, fComputedY), VecToImDraw(fTheta + fDeltaTime, fNewY), IM_COL32(128, 255, 128, 255), 2.5f);

			fComputedY0 = fComputedY;
			fComputedY = fNewY;
		...
		f32 fComputedDY = DFunction(fPosition);
		for (UInt32 uIdx = 0; uIdx <= uIterCount; ++uIdx)
		{
			// Sources:
			...
			{
			case Centered:
				fDY = (0.5f * Function(fTheta + fDeltaTime) - 0.5f * Function(fTheta - fDeltaTime)) / fDeltaTime;
				break;
			case Forward:
				fDY = (Function(fTheta + fDeltaTime) - Function(fTheta)) / fDeltaTime;
				break;
			case Backward:
				fDY = (Function(fTheta) - Function(fTheta - fDeltaTime)) / fDeltaTime;
				break;
			case CenteredOrder6:
			default:
				// Sixth order accuracy - 1st derivative
			...
			//fComputedDY = fComputedDY + fDeltaTime * DDFunction(fTheta, fDY);
			f32 const fNewY = fComputedY + fDeltaTime * fDY;
			//f32 const fNewY = fComputedY + fDeltaTime * fComputedDY;
