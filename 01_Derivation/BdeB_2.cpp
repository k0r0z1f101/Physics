...
void DrawScene(ImDrawList* pDrawList, f32 const dt)
{
	...
	if (bShowIntegration)
	{
		...
		{
			f32 const fDDY = DFunction(fTheta);

			// TODO
			f32 const fNewY = 2.0f * fComputedY - fComputedY + fDeltaTime * fDeltaTime * fDY;
		...
		for (UInt32 uIdx = 0; uIdx <= uIterCount; ++uIdx)
		{
			...
			case Centered:
				...
				break;
			case Forward:
				fDY = (Function(fTheta+ fDeltaTime) - Function(fTheta)) / fDeltaTime; // TODO
				break;
			case Backward:
			...
			default:
				...
				fDY = ((-1.0f / 60.0f) * Function(fTheta - 3.0f * fDeltaTime)
					...
					+ (3.0f / 4.0f) * Function(fTheta + fDeltaTime)
					- (3.0f / 20.0f) * Function(fTheta + 2.0f * fDeltaTime)
					+ (1.0f / 60.0f) * Function(fTheta + 3.0f * fDeltaTime)) / fDeltaTime; // TODO
				break;
			}
