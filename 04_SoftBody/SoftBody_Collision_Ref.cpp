					...
					Scalar const fDeltaTime, Scalar const fSpringDepth)
{
	...
	Scalar const fAngle = Consts::t30 * Consts::tDegToRad;
	Scalar const fCos0 = Func::Cos(fAngle);
	Scalar const fSin0 = Func::Sin(fAngle);
	Mat22 rot = {	fCos0, -fSin0,
					fSin0, fCos0 };

	Vec2 const vCenter = Vec2(Consts::t0, fStartHeight + fStartHeight * Consts::t1_2);
	...
	for (UInt32 j = 0; j < uHeightCount; ++j)
	{
		...
		for (UInt32 i = 0; i < uWidthCount; ++i)
		{
			...
			p.vPreviousPos		= p.vCurrentPos - Vec2(-fSinA, fCosA) * fDeltaTime * glm::length(p.vCurrentPos - vCenter);
...
void DrawUI()
{
	...
	ImGui::DragFloat2("Collider Position", &vColliderPosition.x, 0.125f, -32.0f, 32.0f, "%.3f", ImGuiSliderFlags_Logarithmic);
	ImGui::DragFloat("Collider Radius", &fColliderRadius, 0.125f, -32.0f, 32.0f, "%.3f", ImGuiSliderFlags_Logarithmic);
...
void Update(f32 const dt)
{
	...
	if (bPlay)
	{
		...
		for (Int32 iIdx = 0; iIdx < iSize; ++iIdx)
		{
			...
			Vec2 const vVirtualNextPos = oParticleCur.VirtualStep(fDeltaTime);
			if (vVirtualNextPos.y <= Consts::t0)
			{
				oParticleCur.vExternalForce += Vec2(-oParticleCur.GetSpeed(fDeltaTime).x * fGroundFriction, Consts::t0);
				//oParticleCur.vExternalForce.x += -oParticleCur.GetSpeed(fDeltaTime).x * fGroundFriction;
			}
		}
		#pragma omp parallel for
		for (Int32 iIdx = 0; iIdx < iSize; ++iIdx)
