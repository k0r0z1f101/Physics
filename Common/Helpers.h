...
namespace SK
{
	...
	Vec2	RandomPointOnCircle(Vec2 const vCenter, Scalar const fRadius)
	{
		...
		return Vec2(vCenter.x + fRadius * Func::Cos(vU.y), vCenter.y + fRadius * Func::Sin(vU.y));
	}
	...
	Vec2	RandomPointOnCircleRaw() // Rad [0, 1], Angle [0, Tau]
	{
		Vec2 vU(Func::Sqrt(Func::ReScale(g_oDist(g_oGen), Consts::t_1, Consts::t1, Consts::t0, Consts::t1)),
			Consts::tTau * g_oDist(g_oGen));
		return Vec2(Func::Cos(vU.y), Func::Sin(vU.y));
	}

	inline
	Vec2	RandomPointOnDisk(Scalar const fRadiusSrc, Vec2 const vCenter = Vec2(Consts::t0, Consts::t0))
	{
		Scalar const fRadius = fRadiusSrc * Func::Sqrt(Random(Consts::t0, Consts::t1));
		Scalar const fTheta = Random(Consts::t0, Consts::tTau);

		return Vec2(fRadius * Func::Cos(fTheta), fRadius * Func::Sin(fTheta));
	}
