...
{
	Vec2 const vExtA = d2Ydt2(vExternalForce, mass);
	//Vec2 const vG = Vec2(Consts::t0, g);
	//Vec2 const vA = vExtA + vG;
	//return Consts::t2 * yt - yt_dt + dt * dt * vA;
	return 2.0f * yt - yt_dt + dt * dt * Vec2(vExtA.x, vExtA.y + g);
}
...
void Update(f32 const dt)
{
	...
	if (bContinueUpdate)
	{
		...
		if (vNewPos.y < 0.0f)
		{
			...
			Vec2 const vHit = IntersectSegmentToGround(vNewPos, vCurPos);

			Vec2 const vNewSpeed = glm::reflect(vDelta, Vec2(Consts::t0, Consts::t1));

			aPoints.emplace_back(vHit);
			aPoints.emplace_back(vHit + vNewSpeed);
		}
		else
		{
